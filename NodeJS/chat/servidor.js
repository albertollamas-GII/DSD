...
MongoClient.connect("mongodb://localhost:27017/", { useUnifiedTopology: true }, function (err, db) {
    ...
    dbo.createCollection("chats", function (err, collection) {
        if (err)
            var collection = dbo.collection("chats");
        ...
        io.sockets.on('connection',
            function (client) {
                ...
                //Funcion que envia los estados
                enviarEstado = function(s) {
                    io.sockets.emit('status', s);
                }

                //Obtenemos los chats de mongo
                collection.find().limit(100).sort({_id:1}).toArray(function(err, res) {
                    if (err) {
                        throw err;
                    }
                    //Envio de mensajes
                    io.sockets.emit('salida', res);
                });

                client.on('entrada', function(data){
                    ...
                    if (name == '' || message == '') {
                        //Envia error
                        enviarEstado("Introduzca su nombre y un mensaje");
                    } else {
                        //inserta mensaje
                        collection.insertOne({nombre: name, mensaje: message}, function(){
                            io.sockets.emit('salida', [data]);
                            enviarEstado({
